...
async def test_api_common_multiple_two_simple_numbers(client: AsyncClient):
    response = await client.get("/api/v1/calc/common_multiple", params={"list_numbers": [2, 4]})
...
async def test_api_common_multiple_two_numbers(client: AsyncClient):
    response = await client.get("/api/v1/calc/common_multiple", params={"list_numbers": [3, 40]})
...
async def test_api_common_multiple_many_numbers(client: AsyncClient):
    response = await client.get("/api/v1/calc/common_multiple", params={"list_numbers": [3, 40, 3, 5, 6, 21, 54]})
